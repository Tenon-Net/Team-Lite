...
/// Request body for `POST /api/teams/:id/group-messages`.
#[derive(Debug, Deserialize)]
pub struct SendTeamGroupMessageRequest {
    pub content: String,
    #[serde(default)]
    pub files: Option<Vec<String>>,
    /// Persist files in the shared group timeline without waking any agent.
    #[serde(default)]
    pub share_only: bool,
    /// Empty targets mean "send to the team leader".
    #[serde(default)]
    pub target_slot_ids: Vec<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum TeamGroupMessageKind {
    User,
    Agent,
}

/// A canonical message in the Team group timeline.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct TeamGroupMessage {
    pub id: String,
    pub team_id: String,
    pub kind: TeamGroupMessageKind,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub sender_slot_id: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    pub files: Option<Vec<String>>,
    pub target_slot_ids: Vec<String>,
    /// Per-target delivery results for user messages.
    /// Empty for agent messages and share-only posts. Persisted entries never
    /// carry the full `ack`; use the slim `enqueue_status` / `message_id` /
    /// `team_run_id` fields instead.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub deliveries: Vec<TeamGroupDelivery>,
    pub created_at: TimestampMs,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct TeamGroupDelivery {
    pub slot_id: String,
    pub delivered: bool,
    /// Slim ack summary that survives persistence (unlike `ack`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub enqueue_status: Option<TeamMessageEnqueueStatus>,
    /// Mailbox message id from the delivery ack.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub message_id: Option<String>,
    /// Team run id from the delivery ack.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub team_run_id: Option<String>,
    /// Full ack payload; only present in the live send response and the
    /// `team.groupMessageCreated` deliveries broadcast, never in persisted history.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ack: Option<TeamRunAckResponse>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub error: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct SendTeamGroupMessageResponse {
    pub message: TeamGroupMessage,
    pub deliveries: Vec<TeamGroupDelivery>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct TeamGroupMessageListResponse {
    pub messages: Vec<TeamGroupMessage>,
}
...
pub struct TeamAgentResponse {
    ...
    /// Latest group-chat reply from this agent (truncated preview).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub last_message: Option<String>,
    /// Millisecond timestamp of `last_message`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub last_message_at: Option<TimestampMs>,
}
...
pub struct TeamAgentStatusPayload {
    ...
    pub slot_id: String,
    pub status: String,
    /// Latest group-chat reply preview, attached when the agent settles
    /// (idle/completed) and a message is available.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub last_message: Option<String>,
}
