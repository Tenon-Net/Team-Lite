...
use store::models::{MailboxMessageRow, TeamRow, TeamTaskRow};
use store::{DbError, ITeamRepository, UpdateTaskParams, UpdateTeamParams};
use std::sync::Mutex;
...
pub(crate) mod workspace_harness {
    ...
    impl ITeamRepository for FullMockTeamRepo {
        ...
        async fn write_message(&self, _row: &store::models::MailboxMessageRow) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_message_summary(&self, _message_id: &str, _summary: &str) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_task(&self, _task_id: &str, _params: &store::UpdateTaskParams) -> Result<(), DbError> {
            Ok(())
        }
    ...
    impl IWorkerTaskManager for NoopTaskManager {
        fn get_task(&self, _conversation_id: &str) -> Option<acp::AgentInstance> {
            None
        }
        ...
        async fn get_or_build_task(
            &self,
            _conversation_id: &str,
            _options: acp::types::BuildTaskOptions,
    ...
    impl IAgentMetadataRepository for EmptyAgentMetadataRepo {
        ...
        async fn update_availability_snapshot(
            &self,
            _id: &str,
            _params: &store::models::UpdateAgentAvailabilitySnapshotParams<'_>,
        ) -> Result<Option<AgentMetadataRow>, DbError> {
            Ok(None)
        }
    ...
    pub(crate) fn single_agent_team_request(name: &str) -> CreateTeamRequest {
        CreateTeamRequest {
            name: name.into(),
            agents: vec![api_types::TeamAgentInput {
                name: "Lead".into(),
                role: "lead".into(),
