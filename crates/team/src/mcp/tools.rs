...
use api_types::{TeamToolPermission, TeamToolRole};

// ---------------------------------------------------------------------------
// Tool descriptors (returned by tools/list)
// ---------------------------------------------------------------------------

pub type ToolDescriptor = api_types::TeamToolDescriptor;
...
pub fn all_tool_descriptors_for_role(caller_role: TeammateRole) -> Vec<ToolDescriptor> {
    ...
    api_types::team_tool_descriptors_for_role(role)
}
...
pub fn authorize_tool(caller_role: TeammateRole, tool_name: &str) -> Result<(), String> {
    let Some(spec) = api_types::team_tool_descriptor(tool_name) else {
...
pub fn is_builtin_mcp_backend(backend: &str) -> bool {
    backend == team_common::constants::AIONRS_RUNTIME_BACKEND
}
