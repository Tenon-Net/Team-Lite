...
use api_types::{TeamChildTurnPayload, TeamRunStatus};
...
async fn execute_and_finalize(ctx: &AgentLoopContext, batch: WorkBatch, input: WakeInput) -> ExecuteResult {
    ...
    if outcome.status.is_success()
        && ctx.session.is_group_run_batch(&batch.team_run_ids)
        && let Some(response_text) = outcome.response_text.as_deref()
    {
        ctx.scheduler.set_last_message(&ctx.slot_id, response_text).await;
        if let Err(error) = ctx.session.record_group_response(&ctx.slot_id, response_text).await {
            warn!(
                team_id = %ctx.team_id,
                slot_id = %ctx.slot_id,
                error = %error,
                "failed to record agent response in team group history"
            );
        }
    }
