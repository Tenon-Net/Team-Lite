...
use api_types::{
    ...
    SendTeamGroupMessageRequest, SendTeamGroupMessageResponse, SendTeamMessageRequest, SetModeRequest,
    TeamAgentResponse, TeamGroupMessageListResponse, TeamListResponse, TeamResponse, TeamRunAckResponse,
    TeamRunStateResponse,
};
use team_auth::CurrentUser;
use team_common::ApiError;
use store::DbError;
...
pub fn team_routes(state: TeamRouterState) -> Router {
    Router::new()
        ...
        .route(
            "/api/teams/{id}/group-messages",
            get(list_group_messages).post(send_group_message),
        )
...
async fn list_teams(
    ...
    let teams = if query.archived.unwrap_or(false) {
        state.service.list_archived_teams(&user.id).await?
    } else {
        state.service.list_teams(&user.id).await?
    };
...
async fn archive_team(
    ...
    state.service.archive_team(&user.id, &id).await?;
    Ok(Json(ApiResponse::success()))
}
...
async fn list_group_messages(
    ...
    Extension(user): Extension<CurrentUser>,
    Path(id): Path<String>,
) -> Result<Json<ApiResponse<TeamGroupMessageListResponse>>, ApiError> {
    let messages = state.service.list_group_messages(&user.id, &id).await?;
...
async fn send_group_message(
    ...
    Extension(user): Extension<CurrentUser>,
    Path(id): Path<String>,
    body: Result<Json<SendTeamGroupMessageRequest>, JsonRejection>,
) -> Result<Json<ApiResponse<SendTeamGroupMessageResponse>>, ApiError> {
    let Json(request) = body.map_err(ApiError::from)?;
    let response = state.service.send_group_message(&user.id, &id, request).await?;
