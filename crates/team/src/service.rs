...
use acp::{ActiveLeaseRegistry, AgentError, AgentInstance, IWorkerTaskManager, IdleCleanupCoordinator};
use api_types::{
    AddAgentRequest, CreateTeamRequest, GetConfigOptionsResponse, SendTeamGroupMessageRequest,
    SendTeamGroupMessageResponse, TeamAgentResponse, TeamAgentRuntimeStatus, TeamGroupDelivery, TeamGroupMessage,
    TeamGroupMessageKind, TeamGroupMessageListResponse, TeamResponse, TeamRunAckResponse, TeamRunStateResponse,
    TeamSessionBinding, TeamSessionPhase, TeamSessionStatus, TeamSessionStatusPayload, TeamToolCall,
    TeamToolContextResponse, TeamToolErrorCode, TeamToolErrorPayload, TeamToolTransport, WebSocketMessage,
};
use team_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_id, now_ms};
use store::models::{MailboxMessageRow, TeamRow};
use store::{
...
const TEAM_GROUP_MAILBOX_ID: &str = "__team_group__";
const TEAM_GROUP_USER_SENDER_ID: &str = "__user__";

#[derive(Debug, serde::Serialize, serde::Deserialize)]
struct TeamGroupMessageMetadata {
    kind: TeamGroupMessageKind,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    sender_name: Option<String>,
    target_slot_ids: Vec<String>,
    /// Slim delivery results (never contain the full `ack` payload).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    deliveries: Vec<TeamGroupDelivery>,
}

/// Strips the transient full `ack` payload so only the slim delivery summary
/// (`enqueue_status` / `message_id` / `team_run_id`) is persisted and broadcast
/// as part of the group message.
fn slim_group_delivery(delivery: &TeamGroupDelivery) -> TeamGroupDelivery {
    TeamGroupDelivery {
        slot_id: delivery.slot_id.clone(),
        delivered: delivery.delivered,
        enqueue_status: delivery.enqueue_status.clone(),
...
impl TeamSessionService {
    ...
    async fn load_owned_team_including_archived(&self, user_id: &str, team_id: &str) -> Result<Team, TeamError> {
        let row = self
            .repo
            .get_team(team_id)
            .await?
    ...
    async fn load_owned_team(&self, user_id: &str, team_id: &str) -> Result<Team, TeamError> {
        let team = self.load_owned_team_including_archived(user_id, team_id).await?;
        if team.archived_at.is_some() {
            return Err(TeamError::InvalidRequest(format!("team {team_id} is archived")));
        }
        Ok(team)
    }
    ...
    pub async fn list_teams(&self, user_id: &str) -> Result<Vec<TeamResponse>, TeamError> {
        self.list_teams_from_rows(self.repo.list_teams_by_user(user_id).await?)
            .await
    }

    pub async fn list_archived_teams(&self, user_id: &str) -> Result<Vec<TeamResponse>, TeamError> {
        self.list_teams_from_rows(self.repo.list_archived_teams_by_user(user_id).await?)
            .await
    }

    async fn list_teams_from_rows(&self, rows: Vec<TeamRow>) -> Result<Vec<TeamResponse>, TeamError> {
    ...
    pub async fn archive_team(&self, user_id: &str, team_id: &str) -> Result<(), TeamError> {
        let team = self.load_owned_team_including_archived(user_id, team_id).await?;
        if team.archived_at.is_some() {
            return Ok(());
        }
        ...
        let kill_futures: Vec<_> = team
            .agents
            .iter()
            .map(|agent| self.task_manager.kill_and_wait(&agent.conversation_id, None))
            .collect();
        ...
        self.repo.archive_team(team_id).await?;
        self.add_agent_locks.remove(team_id);
        info!(team_id = %team_id, "Team archived");
        self.broadcast_team_list_changed(team_id, "archived");
        Ok(())
    }
    ...
    async fn ensure_agent_session_inner(&self, team_id: &str, slot_id: &str) -> Result<(), TeamError> {
        let membership_lock = self
            .add_agent_locks
            .entry(team_id.to_owned())
            .or_insert_with(|| Arc::new(tokio::sync::Mutex::new(())))
        ...
        let session = if let Some(entry) = self.sessions.get(team_id) {
            ...
            if let Some(entry) = self.sessions.get(team_id) {
                ...
                let entry = SessionEntry {
                    ...
                    slow_monitor_handle: Self::spawn_slow_monitor(Arc::clone(&session)),
                };
                self.sessions.insert(team_id.to_owned(), entry);
                session
            }
    ...
    async fn ensure_existing_agent_runtime_inner(&self, team_id: &str, slot_id: &str) -> Result<(), TeamError> {
        let membership_lock = self
            .add_agent_locks
            .entry(team_id.to_owned())
            .or_insert_with(|| Arc::new(tokio::sync::Mutex::new(())))
    ...
    pub async fn get_run_state(&self, user_id: &str, team_id: &str) -> Result<TeamRunStateResponse, TeamError> {
        self.load_owned_team_including_archived(user_id, team_id).await?;
        let session = self.sessions.get(team_id).map(|entry| Arc::clone(&entry.session));
        ...
        let active_run = session.team_run_manager().current_payload(&snapshot).filter(|run| {
            matches!(
                run.status,
                api_types::TeamRunStatus::Accepted
                    | api_types::TeamRunStatus::Running
                    | api_types::TeamRunStatus::Cancelling
            )
        });
    ...
    pub async fn resolve_team_tool_context(
        ...
        let allowed_tools = api_types::team_tool_descriptors_for_role(role_to_tool_role(agent.role))
            .into_iter()
    ...
    pub async fn send_message_to_agent(
        ...
        self.load_owned_team(user_id, team_id).await?;
        self.ensure_agent_session_inner(team_id, slot_id).await?;
        let session = {
            let entry = self
                .sessions
                .get(team_id)
    ...
    async fn send_group_message_to_agent(
        &self,
        user_id: &str,
        team_id: &str,
        slot_id: &str,
        ...
        session.send_group_message_to_agent(slot_id, content, files).await
    }
    ...
    pub async fn list_group_messages(
        ...
        user_id: &str,
        team_id: &str,
    ) -> Result<TeamGroupMessageListResponse, TeamError> {
        self.load_owned_team_including_archived(user_id, team_id).await?;
        let rows = self.repo.get_history(team_id, TEAM_GROUP_MAILBOX_ID, None).await?;
        let messages = rows.iter().filter_map(Self::group_message_from_row).collect();
        Ok(TeamGroupMessageListResponse { messages })
    }
    ...
    pub async fn send_group_message(
        ...
        user_id: &str,
        team_id: &str,
        request: SendTeamGroupMessageRequest,
    ) -> Result<SendTeamGroupMessageResponse, TeamError> {
        let team = self.load_owned_team(user_id, team_id).await?;
        let mut target_slot_ids = request.target_slot_ids;
        target_slot_ids.retain(|slot_id| !slot_id.trim().is_empty());
        let mut seen_target_slot_ids = HashSet::new();
        target_slot_ids.retain(|slot_id| seen_target_slot_ids.insert(slot_id.clone()));
        if target_slot_ids.is_empty() && !request.share_only {
            let leader_slot_id = team
                .lead_agent_id
                .clone()
                .or_else(|| {
                    team.agents
                ...
                .ok_or_else(|| TeamError::InvalidRequest("team leader is not configured".into()))?;
            target_slot_ids.push(leader_slot_id);
        }

        let message = self
            .persist_group_message(
                team_id,
                TeamGroupMessageKind::User,
                None,
                None,
                &request.content,
                request.files.clone(),
                target_slot_ids.clone(),
            )
            .await?;

        if request.share_only {
            return Ok(SendTeamGroupMessageResponse {
                message,
                deliveries: Vec::new(),
            });
        }

        let mut deliveries = Vec::with_capacity(target_slot_ids.len());
        for slot_id in &target_slot_ids {
            match self
                .send_group_message_to_agent(user_id, team_id, slot_id, &request.content, request.files.clone())
                .await
            {
                Ok(ack) => deliveries.push(TeamGroupDelivery {
                    slot_id: slot_id.clone(),
                    delivered: true,
                    enqueue_status: Some(ack.enqueue_status.clone()),
                    message_id: Some(ack.message_id.clone()),
                    team_run_id: Some(ack.run.team_run_id.clone()),
                    ack: Some(ack),
                    error: None,
                }),
                Err(error) => {
                    warn!(team_id, slot_id, error = %error, "team group message delivery failed");
                    deliveries.push(TeamGroupDelivery {
                        slot_id: slot_id.clone(),
                        delivered: false,
                        enqueue_status: None,
                        message_id: None,
                        team_run_id: None,
                        ack: None,
                        error: Some("Unable to deliver message".into()),
                    });
                }
            }
        }
        ...
        if !deliveries.is_empty() {
            message.deliveries = deliveries.iter().map(slim_group_delivery).collect();
            let metadata = TeamGroupMessageMetadata {
                kind: message.kind.clone(),
                sender_name: message.sender_name.clone(),
                target_slot_ids: message.target_slot_ids.clone(),
                deliveries: message.deliveries.clone(),
            };
            match serde_json::to_string(&metadata) {
                Ok(summary) => {
                    if let Err(error) = self.repo.update_message_summary(&message.id, &summary).await {
                        warn!(
                            team_id,
                            message_id = %message.id,
                            error = %error,
                            "failed to persist group message deliveries"
                        );
                    }
                }
                Err(error) => warn!(
                    ...
                    message_id = %message.id,
                    error = %error,
                    "failed to serialize group message deliveries"
                ),
            }
            TeamEventEmitter::new(team_id.to_owned(), self.broadcaster.clone())
                .broadcast_group_message(message.clone());
        }

        Ok(SendTeamGroupMessageResponse { message, deliveries })
    }

    pub(crate) async fn persist_agent_group_response(
        &self,
        team_id: &str,
        slot_id: &str,
        content: &str,
        ...
        let sender = team
            ...
            .find(|agent| agent.slot_id == slot_id)
            .ok_or_else(|| TeamError::AgentNotFound(slot_id.into()))?;
        self.persist_group_message(
            team_id,
            TeamGroupMessageKind::Agent,
        ...
        .await?;
        if let Err(error) = self.persist_agent_last_message(team_id, slot_id, content).await {
            warn!(team_id, slot_id, error = %error, "failed to persist agent last message");
        }
        Ok(())
    }

    /// Persists the agent's latest group-chat reply into the `teams.agents`
    /// JSON column. Re-reads the latest team row right before writing to keep
    /// the read-modify-write race window with concurrent member updates small.
    async fn persist_agent_last_message(&self, team_id: &str, slot_id: &str, content: &str) -> Result<(), TeamError> {
        let row = self
            .repo
            .get_team(team_id)
            .await?
            .ok_or_else(|| TeamError::SessionNotFound(team_id.into()))?;
        let team = Team::from_row(&row)?;
        let mut agents = team.agents;
        let Some(agent) = agents.iter_mut().find(|agent| agent.slot_id == slot_id) else {
            return Ok(());
        };
        agent.last_message = Some(crate::types::truncate_last_message(content));
        agent.last_message_at = Some(now_ms());
        let agents_json = serde_json::to_string(&agents)?;
        self.repo
            .update_team(
                team_id,
    ...
    fn group_message_from_row(row: &MailboxMessageRow) -> Option<TeamGroupMessage> {
        let metadata: TeamGroupMessageMetadata = serde_json::from_str(row.summary.as_deref()?).ok()?;
        let files = row
            .files
            .as_deref()
            .and_then(|value| serde_json::from_str::<Vec<String>>(value).ok())
            .filter(|value| !value.is_empty());
        Some(TeamGroupMessage {
            id: row.id.clone(),
            team_id: row.team_id.clone(),
            kind: metadata.kind,
            sender_slot_id: (row.from_agent_id != TEAM_GROUP_USER_SENDER_ID).then(|| row.from_agent_id.clone()),
            sender_name: metadata.sender_name,
            content: row.content.clone(),
            files,
            target_slot_ids: metadata.target_slot_ids,
    ...
    async fn persist_group_message(
        ...
        team_id: &str,
        kind: TeamGroupMessageKind,
        sender_slot_id: Option<String>,
        sender_name: Option<String>,
        content: &str,
        files: Option<Vec<String>>,
        target_slot_ids: Vec<String>,
    ) -> Result<TeamGroupMessage, TeamError> {
        let id = generate_id();
        let created_at = now_ms();
        let metadata = TeamGroupMessageMetadata {
            kind: kind.clone(),
            sender_name: sender_name.clone(),
            target_slot_ids: target_slot_ids.clone(),
            deliveries: Vec::new(),
        };
        let row = MailboxMessageRow {
            id: id.clone(),
            team_id: team_id.to_owned(),
            to_agent_id: TEAM_GROUP_MAILBOX_ID.into(),
            from_agent_id: sender_slot_id
                .clone()
                .unwrap_or_else(|| TEAM_GROUP_USER_SENDER_ID.into()),
            msg_type: "message".into(),
            content: content.to_owned(),
            summary: Some(serde_json::to_string(&metadata)?),
            files: files
                .as_ref()
                .filter(|value| !value.is_empty())
        ...
        TeamEventEmitter::new(team_id.to_owned(), self.broadcaster.clone()).broadcast_group_message(message.clone());
        Ok(message)
    }
    ...
    pub async fn cancel_run(
        ...
        let session = {
            let entry = self
                .sessions
                .get(team_id)
                .ok_or_else(|| TeamError::InvalidRequest(format!("team run {team_run_id} is not active")))?;
            Arc::clone(&entry.session)
        };
    ...
    pub async fn send_agent_message_from_agent(
        ...
        self.ensure_existing_agent_runtime_inner(team_id, to_slot_id).await?;
        let session = {
            let entry = self
                .sessions
                .get(team_id)
        ...
        let sender = self
            ...
            .ok()
            .flatten()
            .and_then(|row| Team::from_row(&row).ok())
            .and_then(|team| team.agents.into_iter().find(|agent| agent.slot_id == from_slot_id));
        if let Some(sender) = sender.filter(|agent| agent.role == TeammateRole::Lead) {
            if let Err(error) = self
                .persist_group_message(
                    team_id,
                    TeamGroupMessageKind::Agent,
                    Some(from_slot_id.to_owned()),
                    Some(sender.name),
                    content,
                    None,
                    vec![to_slot_id.to_owned()],
                )
                .await
            {
                warn!(team_id, from_slot_id, to_slot_id, error = %error, "failed to record team delegation in group history");
            }
        }
        Ok(result)
    }
...
mod tests {
    ...
    use acp::types::{BuildTaskOptions, SendMessageData};
    use acp::{
    ...
    use api_types::{AddAgentRequest, ConfigOptionConfirmation, SetConfigOptionResponse};
    use team_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
    use store::{IConversationRepository, ITeamRepository};
    ...
    fn two_agent_team_request(name: &str) -> api_types::CreateTeamRequest {
        api_types::CreateTeamRequest {
            name: name.into(),
            agents: vec![
                api_types::TeamAgentInput {
                    name: "Lead".into(),
                    role: "lead".into(),
    ...
    fn team_with_aionrs_worker_request(name: &str) -> api_types::CreateTeamRequest {
        let mut request = two_agent_team_request(name);
        request.agents.push(api_types::TeamAgentInput {
            name: "Butler".into(),
            role: "teammate".into(),
    ...
    async fn set_session_mode_does_not_persist_agent_seed_when_active_runtime_rejects_mode() {
        ...
        .unwrap();
        conv_repo
            .create(&store::models::ConversationRow {
