...
use api_types::{
    ...
    TeamAgentSpawnedPayload, TeamAgentStatusPayload, TeamChildTurnPayload, TeamGroupMessage, TeamRunPayload,
    WebSocketMessage,
};
...
pub const TEAM_GROUP_MESSAGE_CREATED_EVENT: &str = "team.groupMessageCreated";
...
impl TeamEventEmitter {
    ...
    pub fn broadcast_agent_status(&self, slot_id: &str, status: TeammateStatus, last_message: Option<String>) {
    ...
    pub fn broadcast_group_message(&self, payload: TeamGroupMessage) {
        let event = WebSocketMessage::new(
            TEAM_GROUP_MESSAGE_CREATED_EVENT,
            serde_json::to_value(payload).expect("serialize team group message payload"),
        );
        self.broadcaster.broadcast(event);
    }
}
...
mod tests {
    ...
    fn status_event_has_correct_shape() {
        ...
        emitter.broadcast_agent_status("slot-1", TeammateStatus::Working, None);
    ...
    fn multiple_events_accumulate() {
        let (emitter, bc) = make_emitter();
        emitter.broadcast_agent_status("s1", TeammateStatus::Working, None);
        emitter.broadcast_agent_status("s1", TeammateStatus::Idle, None);
    ...
    fn team_run_event_has_correct_shape() {
        ...
        emitter.broadcast_team_run(
            ...
            api_types::TeamRunPayload {
                team_id: "team-1".into(),
                team_run_id: "run-1".into(),
                source: api_types::TeamRunSource::UserMessage,
                has_user_intervention: true,
                target_slot_id: "lead-1".into(),
                target_role: api_types::TeamRunTargetRole::Lead,
                status: api_types::TeamRunStatus::Accepted,
        ...
        let payload: api_types::TeamRunPayload = serde_json::from_value(events[0].data.clone()).unwrap();
        assert_eq!(payload.team_id, "team-1");
        assert_eq!(payload.team_run_id, "run-1");
        assert_eq!(payload.target_role, api_types::TeamRunTargetRole::Lead);
        assert_eq!(payload.status, api_types::TeamRunStatus::Accepted);
    ...
    fn child_turn_event_has_correct_shape() {
        ...
        emitter.broadcast_child_turn(
            ...
            api_types::TeamChildTurnPayload {
                team_id: "team-1".into(),
                team_run_id: "run-1".into(),
                slot_id: "worker-1".into(),
                role: api_types::TeamRunTargetRole::Teammate,
                conversation_id: "conv-1".into(),
                turn_id: "turn-1".into(),
                status: api_types::TeamRunStatus::Running,
            },
        );
        ...
        let payload: api_types::TeamChildTurnPayload = serde_json::from_value(events[0].data.clone()).unwrap();
        assert_eq!(payload.team_id, "team-1");
        assert_eq!(payload.team_run_id, "run-1");
        assert_eq!(payload.slot_id, "worker-1");
        assert_eq!(payload.status, api_types::TeamRunStatus::Running);
    }
    ...
    fn all_status_variants_serialize() {
        ...
        for s in statuses {
            emitter.broadcast_agent_status("s1", s, None);
        }
