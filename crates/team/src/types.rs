...
use api_types::{TeamAgentResponse, TeamResponse};
...
/// Maximum number of characters persisted for an agent's `last_message` preview.
pub(crate) const TEAM_AGENT_LAST_MESSAGE_MAX_CHARS: usize = 500;

/// Truncates a group-chat reply to the persisted preview length.
pub(crate) fn truncate_last_message(content: &str) -> String {
    content.chars().take(TEAM_AGENT_LAST_MESSAGE_MAX_CHARS).collect()
}
