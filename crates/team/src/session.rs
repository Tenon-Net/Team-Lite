...
use team_common::{AgentKillReason, generate_id};
use store::ITeamRepository;
...
impl TeamSession {
    ...
    pub async fn start_with_prompt_dump(
        ...
        let auth_token = team_common::generate_id();
    ...
    pub(crate) fn mark_group_run(&self, team_run_id: &str) {
        self.group_run_ids.insert(team_run_id.to_owned());
    }

    pub(crate) fn is_group_run_batch(&self, team_run_ids: &[String]) -> bool {
        team_run_ids
            .iter()
            .any(|team_run_id| self.group_run_ids.contains(team_run_id))
    }

    pub(crate) async fn record_group_response(&self, slot_id: &str, content: &str) -> Result<(), TeamError> {
        let service = self
            .service
            .upgrade()
            .ok_or_else(|| TeamError::InvalidRequest("team service is unavailable".into()))?;
        service
            .persist_agent_group_response(&self.team.id, slot_id, content)
            .await
    }
    ...
    pub async fn send_message(
        ...
        self.enqueue_user_message(&lead_slot_id, TeamRunTargetRole::Lead, content, files, false)
            .await
    }
    ...
    pub async fn send_message_to_agent(
        ...
        let agent = self.scheduler.get_agent(slot_id).await?;
        self.enqueue_user_message(slot_id, target_role_for(agent.role), content, files, false)
            .await
    }

    pub async fn send_group_message_to_agent(
        &self,
        slot_id: &str,
        content: &str,
        files: Option<Vec<String>>,
        ...
        let agent = self.scheduler.get_agent(slot_id).await?;
        self.enqueue_user_message(slot_id, target_role_for(agent.role), content, files, true)
            .await
    }
    ...
    async fn enqueue_user_message(
        ...
        let commit = self
            ...
            .await?;
        if group_origin && let Some(team_run_id) = commit.team_run_id.as_deref() {
            self.mark_group_run(team_run_id);
        }
...
mod tests {
    ...
    use acp::AgentError;
    use acp::agent_task::AgentInstance;
    use acp::types::BuildTaskOptions;
    use api_types::WebSocketMessage;
    use team_common::{AgentKillReason, TimestampMs};
    ...
    impl TeamProjectionMessageStore for NoopProjectionStore {
        ...
        async fn find_projected_message(
            ...
            _msg_id: &str,
            _msg_type: &str,
        ) -> Result<Option<store::models::MessageRow>, TeamError> {
            Ok(None)
        }

        async fn insert_projected_message(&self, _row: &store::models::MessageRow) -> Result<(), TeamError> {
            Ok(())
        }
    }
    ...
    async fn message_to_runtime_starting_member_returns_blocked_runtime_starting() {
        ...
        assert_eq!(
            ...
            Some(api_types::TeamSlotBlockedReason::RuntimeStarting)
        );
        session.stop();
    }
