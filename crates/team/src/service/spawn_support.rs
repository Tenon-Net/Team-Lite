use super::*;
use team_common::AgentType;
use store::models::AgentMetadataRow;
use store::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
use std::sync::Arc;
...
impl TeamSessionService {
    ...
    /// Provider table removed (ADR 0003 / C2). aionrs no longer resolves models here.
...
mod tests {
    ...
    use store::models::{AgentMetadataRow, AssistantDefinitionRow, AssistantOverlayRow};
    use store::{
    ...
    async fn resolve_spawn_backend_and_model_prefers_assistant_identity_over_caller_backend() {
        ...
        // ADR 0003 / C2: no provider table — aionrs has no backend default model,
        // so spawn falls back to the caller-supplied fallback model.
