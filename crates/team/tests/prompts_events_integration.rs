...
use team::events::TeamEventEmitter;
use team::message_projection::{
    ProjectedTeamMessage, TeamMessageProjection, TeamProjectionMessageStore, TeamProjectionRequest,
    TeamProjectionSource,
};
use team::prompts::{AvailableAssistant, build_lead_prompt, build_teammate_prompt, build_wake_payload};
use team::types::{
    MailboxMessage, MailboxMessageType, TaskStatus, TeamAgent, TeamTask, TeammateRole, TeammateStatus,
};
use team::visibility::TeamVisibilityPolicy;
use team::{Mailbox, TaskBoard, TeammateManager};
...
fn event_emitter_uses_typed_payloads() {
    ...
    emitter.broadcast_agent_status("s1", TeammateStatus::Thinking, None);
