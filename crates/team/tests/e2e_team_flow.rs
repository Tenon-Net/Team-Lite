...
use acp::AgentError;
use acp::agent_task::{AgentInstance, IAgentTask, IMockAgent};
use acp::protocol::events::{AgentStreamEvent, FinishEventData};
use acp::shared_kernel::approval_key;
use acp::types::{BuildTaskOptions, SendMessageData};
use api_types::AgentModeResponse;
use api_types::WebSocketMessage;
use team_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs, now_ms};
use store::ITeamRepository;
use store::EventBroadcaster;
use team::event_loop::AgentLoopContext;
use team::mcp::protocol::{read_frame, write_frame};
use team::ports::{
    AgentTurnCancellationPort, AgentTurnExecutionError, AgentTurnExecutionPort, AgentTurnOutcome, AgentTurnRequest,
    AgentTurnSource, AgentTurnStarted, AgentTurnStatus,
};
use team::service::TeamSessionService;
use team::{TeamAgent, TeamProjectionMessageStore, TeamSession, TeammateRole};
...
impl TeamProjectionMessageStore for NoopProjectionStore {
    ...
    async fn find_projected_message(
        ...
        _msg_id: &str,
        _msg_type: &str,
    ) -> Result<Option<store::models::MessageRow>, team::TeamError> {
        Ok(None)
    }

    async fn insert_projected_message(
        &self,
        _row: &store::models::MessageRow,
...
impl IAgentTask for RecordingAgent {
    ...
    async fn send_message(&self, data: SendMessageData) -> Result<(), acp::AgentSendError> {
        self.sent.lock().unwrap().push(data);
        match &self.fail_with {
            Some(msg) => Err(acp::AgentSendError::from_agent_error(AgentError::internal(
                msg.clone(),
            ))),
            None => Ok(()),
        }
...
#[async_trait]
impl acp::IWorkerTaskManager for StubTaskManager {
...
async fn setup_session_with_turn_recorder_inner(
    ...
    let task_manager_dyn: Arc<dyn acp::IWorkerTaskManager> = task_manager.clone();
...
async fn setup_session_with_runtime_ports(
    ...
    let task_manager_dyn: Arc<dyn acp::IWorkerTaskManager> = task_manager;
    let team = team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
        workspace: "/tmp/e2e-team".into(),
...
async fn s3a_on_agent_finish_writes_idle_notification_to_lead() {
    ...
    session
        .scheduler()
        .set_status("worker-1", team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3b_lead_finish_does_not_write_idle_notification() {
    ...
    session
        .scheduler()
        .set_status("lead-1", team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3c_finish_triggers_lead_wake_with_idle_notification() {
    ...
    session
        ...
        .write(
            ...
            "worker-1",
            "lead-1",
            team::MailboxMessageType::Message,
            "do the work",
            None,
        )
        .await
...
async fn one_notify_drains_five_intents_after_busy_turn() {
    ...
    for index in 1..=5 {
        ...
        assert_eq!(ack.enqueue_status, api_types::TeamMessageEnqueueStatus::Queued);
