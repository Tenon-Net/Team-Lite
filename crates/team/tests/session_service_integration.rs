...
use acp::session_context::{
    AcpSessionBuildContext, AgentSessionContext, AgentSessionKind, ConversationContext, WorkspaceContext,
};
use acp::task_manager::AgentFactory;
use acp::types::BuildTaskOptions;
use acp::{ActiveLeaseRegistry, AgentError, IWorkerTaskManager, WorkerTaskManagerImpl};
use api_types::{
    AcpBuildExtra, AcpConfigOptionDto, AcpConfigSelectOptionDto, AddAgentRequest, CreateTeamRequest,
    GetConfigOptionsResponse, SendTeamGroupMessageRequest, TeamAgentInput, WebSocketMessage,
};
use team_common::{AgentKillReason, AgentType, PaginatedResult, ProviderWithModel};
use store::models::{
...
use store::{
    ...
    IAssistantOverlayRepository, IConversationRepository, ITeamRepository, MessagePageParams,
    MessagePageResult, MessageRowUpdate, MessageSearchRow, resolve_agent_binding_from_rows,
};
use store::EventBroadcaster;
...
impl RecordingTurnPort {
    fn set_response_text(&self, response_text: impl Into<String>) {
        *self.response_text.lock().unwrap() = Some(response_text.into());
    }
}
...
impl AgentTurnExecutionPort for RecordingTurnPort {
    async fn run_agent_turn(&self, request: AgentTurnRequest) -> Result<AgentTurnOutcome, AgentTurnExecutionError> {
        ...
        Ok(AgentTurnOutcome {
            ...
            status: AgentTurnStatus::Completed,
            response_text: self.response_text.lock().unwrap().clone(),
            runtime: None,
        })
    }
}
...
impl FakeConversationPorts {
    fn new(repo: Arc<MockConversationRepo>) -> Self {
        let workspace_root =
            std::env::temp_dir().join(format!("aionui-team-fake-workspaces-{}", team_common::generate_id()));
        Self {
            repo,
            workspace_root,
...
impl TeamConversationProvisioningPort for FakeConversationPorts {
    ...
    async fn conversation_workspace(&self, conversation_id: &str) -> Result<Option<String>, team::TeamError> {
    ...
    async fn conversation_assistant_id(&self, conversation_id: &str) -> Result<Option<String>, team::TeamError> {
        Ok(self.repo.get_extra(conversation_id).and_then(|extra| {
            extra
                .get("assistant_id")
                .or_else(|| extra.get("preset_assistant_id"))
    ...
    async fn create_team_temp_workspace(&self, team_id: &str) -> Result<String, team::TeamError> {
        if self
        ...
        self.repo
            .update(
                ...
                &ConversationRowUpdate {
                    ...
                    status: None,
                    updated_at: Some(team_common::now_ms()),
                },
            )
            .await?;
        Ok(())
    ...
    async fn save_acp_runtime_mode(&self, conversation_id: &str, mode: &str) -> Result<(), team::TeamError> {
    ...
    async fn get_config_options(
        &self,
        conversation_id: &str,
    ) -> Result<GetConfigOptionsResponse, team::TeamError> {
        let extra = self
            .repo
            .get_extra(conversation_id)
            .ok_or_else(|| team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
        let model = extra
    ...
    async fn warmup_agent_process(
        ...
        let row = self
            ...
            .ok_or_else(|| {
                team::TeamError::InvalidRequest(format!("conversation not found: {conversation_id}"))
            })?;
        let extra: serde_json::Value = serde_json::from_str(&row.extra)?;
        let team = api_types::TeamSessionBinding::from_extra_value(&extra)?;
        ...
        task_manager
            ...
            .await
            .map_err(|error| {
                team::TeamError::InvalidRequest(format!("failed to warm up agent process: {error}"))
            })?;
        Ok(())
    }
...
impl TeamProjectionMessageStore for FakeConversationPorts {
    ...
    async fn find_projected_message(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, team::TeamError> {
        Ok(self
            .repo
    ...
    async fn insert_projected_message(&self, row: &MessageRow) -> Result<(), team::TeamError> {
...
impl TeamConversationLookupPort for FakeConversationPorts {
    async fn lookup_team_binding_by_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<TeamConversationBindingLookup>, team::TeamError> {
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<store::models::TeamRow>>,
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &store::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams(&self) -> Result<Vec<store::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn list_teams_by_user(&self, user_id: &str) -> Result<Vec<store::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team(&self, id: &str) -> Result<Option<store::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().iter().find(|t| t.id == id).cloned())
    }
    async fn update_team(&self, id: &str, params: &store::UpdateTeamParams) -> Result<(), DbError> {
        ...
        team.updated_at = team_common::now_ms();
        Ok(())
    }
    ...
    async fn write_message(&self, row: &store::models::MailboxMessageRow) -> Result<(), DbError> {
    ...
    async fn read_unread_and_mark(
        ...
        team_id: &str,
        to_agent_id: &str,
    ) -> Result<Vec<store::models::MailboxMessageRow>, DbError> {
    ...
    async fn update_message_summary(&self, message_id: &str, summary: &str) -> Result<(), DbError> {
        self.inner.update_message_summary(message_id, summary).await
    }
    ...
    async fn create_task(&self, row: &store::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<store::models::TeamTaskRow>, DbError> {
        self.inner.find_task_by_id(team_id, task_id).await
    }
    async fn update_task(&self, task_id: &str, params: &store::UpdateTaskParams) -> Result<(), DbError> {
        self.inner.update_task(task_id, params).await
    }
    async fn list_tasks(&self, team_id: &str) -> Result<Vec<store::models::TeamTaskRow>, DbError> {
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<acp::AgentInstance> {
    ...
    ) -> Result<acp::AgentInstance, AgentError> {
    ...
    fn collect_idle(&self, idle_threshold_ms: team_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    use acp::AgentError;
    use acp::agent_task::{IAgentTask, IMockAgent};
    use acp::protocol::events::AgentStreamEvent;
    use acp::types::SendMessageData;
    use team_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs};
    ...
    impl IAgentTask for MockAgent {
        ...
        async fn send_message(&self, _data: SendMessageData) -> Result<(), acp::AgentSendError> {
            Ok(())
        }
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(acp::AgentInstance::Mock(Arc::new(
...
fn test_acp_build_options(conversation_id: String, workspace: String) -> BuildTaskOptions {
    BuildTaskOptions::new(AgentSessionContext {
        conversation: ConversationContext {
            conversation_id,
            user_id: "user1".into(),
            agent_type: team_common::AgentType::Acp,
            source: None,
        },
        workspace: WorkspaceContext {
            path: workspace.clone(),
...
async fn recovery_creates_background_intents_without_restoring_old_memory_run() {
    ...
    team_repo
        .write_message(&store::models::MailboxMessageRow {
...
async fn renew_active_lease_allows_empty_team_without_unrelated_lease() {
    ...
    team_repo
        .create_team(&store::models::TeamRow {
            id: "team-empty".into(),
            user_id: "user1".into(),
            name: "Empty".into(),
...
async fn create_team_with_workspace_writes_same_workspace_to_team_and_initial_agents() {
    ...
    let workspace_dir =
        std::env::temp_dir().join(format!("aionui-team-user-workspace-{}", team_common::generate_id()));
...
async fn tc_create_team_carries_assistant_identity_into_lead_conversation_extra() {
    ...
    assert_eq!(extra["provider_id"], serde_json::json!("provider-claude-cpa"));
    assert_eq!(extra["current_model_id"], serde_json::json!("claude"));
...
async fn tg3_get_team_rejects_cross_user_access() {
    ...
    assert!(matches!(result, Err(team::TeamError::Forbidden(_))));
}
...
async fn aa_add_agent_inherits_team_workspace() {
    ...
    let (svc, _, conv_repo) =
        setup_with_factory_and_metadata_and_conversation_repo(success_factory(), agent_metadata_repo);
    let workspace = std::env::temp_dir().join(format!("aionui-team-workspace-{}", team_common::generate_id()));
...
#[tokio::test]
async fn provisioning_does_not_bind_grok_cli_backend_as_provider_id() {
    let agent_metadata_repo: Arc<dyn IAgentMetadataRepository> =
    ...
    assert_eq!(extra.get("backend").and_then(serde_json::Value::as_str), Some("grok"));
    assert_eq!(
    ...
    assert!(
        ...
        "the Grok CLI backend name is not a provider repository ID"
    );
}
...
async fn d31b2_ensure_session_broadcasts_failed_loading_team_for_missing_team() {
    ...
    assert!(matches!(err, team::TeamError::TeamNotFound(_)));
...
#[tokio::test]
async fn send_message_to_lead_starts_only_the_lead_runtime() {
    use futures_util::FutureExt;

    let starts = Arc::new(AtomicUsize::new(0));
    let starts_for_factory = Arc::clone(&starts);
    let factory: AgentFactory = Arc::new(move |opts: BuildTaskOptions| {
        let starts = Arc::clone(&starts_for_factory);
        async move {
            if starts.fetch_add(1, Ordering::SeqCst) > 0 {
                return Err(AgentError::internal("non-target runtime must not start"));
            }
            Ok(acp::AgentInstance::Mock(Arc::new(
                mock_agent::MockAgent::new(opts.context.conversation.conversation_id, opts.context.workspace.path),
            )))
    ...
    let (svc, task_manager) = setup_with_factory(factory);
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Targeted lead wake".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
        .await
        .unwrap();
    reset_runtime_state(&svc, &task_manager, &created.id).await;
    let lead = &created.assistants[0];

    svc.send_message_to_agent("user1", &created.id, &lead.slot_id, "Handle this directly", None)
        .await
        .unwrap();
...
#[tokio::test]
async fn group_message_without_mentions_starts_only_the_leader_runtime() {
    let (svc, task_manager) = setup_with_factory(success_factory());
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Leader-default group".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    let response = svc
        .send_group_message(
            ...
            SendTeamGroupMessageRequest {
                content: "Coordinate this".into(),
                files: None,
                share_only: false,
                target_slot_ids: Vec::new(),
            },
        )
        .await
        .unwrap();

    assert_eq!(response.message.target_slot_ids, vec![lead.slot_id.clone()]);
    assert_eq!(response.deliveries.len(), 1);
    assert!(response.deliveries[0].delivered);
    assert_eq!(task_manager.snapshot().build, vec![lead.conversation_id.clone()]);
}

#[tokio::test]
async fn group_message_with_explicit_target_starts_only_that_member_runtime() {
    let (svc, task_manager) = setup_with_factory(success_factory());
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Targeted group".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    svc.send_group_message(
        "user1",
        &created.id,
        SendTeamGroupMessageRequest {
            content: "Handle this".into(),
            files: None,
            share_only: false,
            target_slot_ids: vec![worker.slot_id.clone(), worker.slot_id.clone()],
        },
    )
    .await
    .unwrap();
    ...
    let history = svc.list_group_messages("user1", &created.id).await.unwrap();
    assert_eq!(history.messages.len(), 1);
    assert_eq!(history.messages[0].target_slot_ids, vec![worker.slot_id.clone()]);
}

#[tokio::test]
async fn group_member_response_is_persisted_in_group_history() {
    let (svc, _team_repo, turn_port, _conv_repo) = setup_with_recording_turn_port();
    turn_port.set_response_text("Worker finished");
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Visible group response".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    svc.send_group_message(
        ...
        SendTeamGroupMessageRequest {
            ...
            files: None,
            share_only: false,
            target_slot_ids: vec![worker.slot_id.clone()],
        },
    )
    .await
    .unwrap();
    ...
    .await
    .expect("member response should be recorded in group history");

    let response = history
        .messages
        .iter()
        .find(|message| message.kind == api_types::TeamGroupMessageKind::Agent)
        .expect("agent response should be present");
    assert_eq!(response.sender_slot_id.as_deref(), Some(worker.slot_id.as_str()));
    assert_eq!(response.sender_name.as_deref(), Some(worker.assistant_name.as_str()));
    assert_eq!(response.content, "Worker finished");
    assert!(response.target_slot_ids.is_empty());
}

#[tokio::test]
async fn share_only_group_message_persists_files_without_starting_agents() {
    let (svc, task_manager) = setup_with_factory(success_factory());
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Shared project files".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    let response = svc
        .send_group_message(
            ...
            &created.id,
            SendTeamGroupMessageRequest {
                content: "Shared 1 project file".into(),
                files: Some(vec!["/workspace/brief.pdf".into()]),
                share_only: true,
                target_slot_ids: Vec::new(),
            },
        )
    ...
    assert!(response.message.target_slot_ids.is_empty());
    assert_eq!(response.message.files, Some(vec!["/workspace/brief.pdf".into()]));
    assert!(task_manager.snapshot().build.is_empty());
    let history = svc.list_group_messages("user1", &created.id).await.unwrap();
    assert_eq!(history.messages.len(), 1);
    assert_eq!(history.messages[0].files, Some(vec!["/workspace/brief.pdf".into()]));
}

#[tokio::test]
async fn group_message_to_everyone_starts_each_member_once() {
    let (svc, task_manager) = setup_with_factory(success_factory());
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Everyone group".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
        ...
        .unwrap();
    reset_runtime_state(&svc, &task_manager, &created.id).await;
    let target_slot_ids = created
        .assistants
        .iter()
        .map(|assistant| assistant.slot_id.clone())
        .collect();
    ...
    let response = svc
        .send_group_message(
            ...
            &created.id,
            SendTeamGroupMessageRequest {
                content: "Everyone handle this".into(),
                files: None,
                share_only: false,
                target_slot_ids,
            },
    ...
    assert_eq!(response.deliveries.len(), created.assistants.len());
    assert!(response.deliveries.iter().all(|delivery| delivery.delivered));
    assert_eq!(
        task_manager.snapshot().build,
        created
            .assistants
            .iter()
...
#[tokio::test]
async fn group_history_is_separate_from_private_member_messages() {
    let (svc, _team_repo, turn_port, _conv_repo) = setup_with_recording_turn_port();
    turn_port.set_response_text("Private response");
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Private history isolation".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    svc.send_message_to_agent("user1", &created.id, &worker.slot_id, "Private instruction", None)
        .await
        .unwrap();
    tokio::task::yield_now().await;
...
#[tokio::test]
async fn pause_without_a_live_session_does_not_wake_the_team() {
    let (svc, task_manager) = setup_with_factory(success_factory());
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Stopped team".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    let result = svc
        .pause_slot_work(
            ...
            &created.id,
            "stale-run",
            &created.assistants[0].slot_id,
            Some("user_stop".into()),
        )
        .await;

    assert!(matches!(result, Err(TeamError::InvalidRequest(message)) if message == "no active team run to pause"));
    assert!(task_manager.snapshot().build.is_empty());
}

#[tokio::test]
async fn group_messages_reject_cross_user_read_and_send() {
    let svc = setup();
    let created = svc
        .create_team(
            "user1",
    ...
    let read_result = svc.list_group_messages("user2", &created.id).await;
    assert!(matches!(read_result, Err(TeamError::Forbidden(_))));
    ...
    let send_result = svc
        ...
        .await;
    assert!(matches!(send_result, Err(TeamError::Forbidden(_))));
}

#[tokio::test]
async fn failed_group_delivery_keeps_canonical_history() {
    let svc = setup();
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Durable group history".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    let response = svc
        .send_group_message(
            ...
            SendTeamGroupMessageRequest {
                ...
                files: None,
                share_only: false,
                target_slot_ids: vec!["deleted-slot".into()],
            },
        )
        .await
        .unwrap();
    ...
    assert!(!response.deliveries[0].delivered);
    assert_eq!(
        response.deliveries[0].error.as_deref(),
        Some("Unable to deliver message")
    );
    let history = svc.list_group_messages("user1", &created.id).await.unwrap();
    assert_eq!(history.messages.len(), 1);
    assert_eq!(history.messages[0].content, "Keep this message");
}

#[tokio::test]
async fn leader_delegation_starts_only_the_explicit_target_runtime() {
    let (svc, task_manager) = setup_with_factory(success_factory());
    let created = svc
        .create_team(
            "user1",
            CreateTeamRequest {
                name: "Lazy delegation".into(),
                agents: two_agent_input(),
                workspace: None,
            },
        )
    ...
    svc.send_message_to_agent("user1", &created.id, &lead.slot_id, "Plan this", None)
        .await
        .unwrap();
    svc.send_agent_message_from_agent(&created.id, &lead.slot_id, &worker.slot_id, "Do this", None)
        .await
        .unwrap();
    ...
    assert_eq!(
        calls.build,
        vec![lead.conversation_id.clone(), worker.conversation_id.clone()]
    );
}
