...
import { resolveDuplicateAssistantOption, type TeamAssistantOption } from './assistantSelectUtils';
...
const TeamCreateModal: React.FC<Props> = ({ visible, onClose, onCreated, sourceTeam }) => {
  const { t, i18n } = useTranslation();
  const { user } = useAuth();
  const layout = useLayoutContext();
  const isMobile = layout?.isMobile ?? false;
  const { assistants: allAssistants, loading: assistantsLoading } = useTeamAssistantOptions(i18n?.language ?? 'en-US');
  ...
  const [unavailableMemberNames, setUnavailableMemberNames] = useState<string[]>([]);
  ...
  const duplicateInitializationRef = useRef<string | null>(null);
  const isDuplicate = Boolean(sourceTeam);

  useEffect(() => {
    if (!visible) {
      duplicateInitializationRef.current = null;
      return;
    }
    if (!sourceTeam || assistantsLoading) return;

    const assistantCatalogSignature = allAssistants
      .map((assistant) =>
        [assistant.id, assistant.name, assistant.backend, assistant.icon, assistant.team_selectable].join(':')
      )
      .join('|');
    const initializationKey = `${sourceTeam.id}:${i18n?.language ?? 'en-US'}:${assistantCatalogSignature}`;
    if (duplicateInitializationRef.current === initializationKey) return;

    const unavailable: string[] = [];
    const members = sourceTeam.assistants.flatMap<TeamMemberDraft>((sourceMember) => {
      const assistant = resolveDuplicateAssistantOption(sourceMember, allAssistants);
      if (!assistant) {
        unavailable.push(sourceMember.assistant_name);
        return [];
      }
      return [
        {
          selectionId: `${sourceTeam.id}-${sourceMember.slot_id}`,
          assistant: {
            ...assistant,
            name: sourceMember.assistant_name || assistant.name,
          },
        },
      ];
    });
    const sourceLeader = sourceTeam.assistants.find((assistant) => assistant.role === 'leader');

    setName(
      t('team.duplicate.defaultName', {
        name: sourceTeam.name,
        defaultValue: `${sourceTeam.name} Copy`,
      })
    );
    setWorkspace('');
    setSelectedMembers(members);
    setLeaderSelectionId(sourceLeader ? `${sourceTeam.id}-${sourceLeader.slot_id}` : undefined);
    setUnavailableMemberNames(unavailable);
    duplicateInitializationRef.current = initializationKey;
  }, [allAssistants, assistantsLoading, i18n?.language, sourceTeam, t, visible]);
  ...
  const handleCreate = async () => {
    ...
    if (isDuplicate && !workspace.trim()) {
      Message.warning(
        t('team.duplicate.workspaceRequired', {
          defaultValue: 'Select a workspace for the copied team.',
        })
      );
      return;
    }
    if (unavailableMemberNames.length > 0) return;
  ...
  const teamFields = (
    <div className='grid grid-cols-[76px_minmax(0,1fr)] items-center gap-x-14px gap-y-10px'>
      ...
      {isDuplicate && unavailableMemberNames.length > 0 ? (
        <div className='col-span-2'>
          <Alert
            type='warning'
            content={t('team.duplicate.unavailableMembers', {
              names: unavailableMemberNames.join(', '),
              defaultValue: `These members are unavailable and cannot be copied: ${unavailableMemberNames.join(', ')}`,
            })}
          />
        </div>
      ) : null}
  ...
  return (
    <AionModal
      ...
      header={{
        title: isDuplicate
          ? t('team.duplicate.title', { defaultValue: 'Copy Team' })
          : t('team.create.title', { defaultValue: 'New Team' }),
        subtitle: isDuplicate
          ? t('team.duplicate.subtitle', {
              defaultValue:
                'Keep the team members and choose a workspace for the copy. Chat history, tasks, and runtime state are not copied.',
            })
