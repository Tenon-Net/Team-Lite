...
export function useTeamList() {
  ...
  const { data: archivedTeams = [], mutate: mutateArchived } = useSWR<TTeam[]>(
    `teams/${user_id}/archived`,
    () => ipcBridge.team.listArchived.invoke({ user_id }),
    { revalidateOnFocus: false }
  );
  ...
  const deleteArchivedTeam = useCallback(
    async (id: string) => {
      const team = archivedTeams.find((item) => item.id === id);
      if (team) {
        await removeTeamWithCronCleanup({
          team,
          getConversation: getConversationOrNull,
    ...
    [archivedTeams, mutateArchived]
  );

  return { teams, archivedTeams, mutate, mutateArchived, removeTeam, deleteArchivedTeam };
}
