...
export type TeamTabsContextValue = {
  ...
  syncingSlotIds: Set<string>;
  relayingSlotIds: Set<string>;
  team_id: string;
  ...
  syncAssistant?: (slot_id: string) => void;
  relayAssistant?: (slot_id: string) => void;
  syncAllAssistants?: () => void;
...
const EMPTY_RELAYING_SLOT_IDS = new Set<string>();
...
export const TeamTabsProvider: React.FC<{
  ...
  statusMap,
  syncingSlotIds,
  relayingSlotIds = EMPTY_RELAYING_SLOT_IDS,
  defaultActiveSlotId,
  team_id,
  renameAssistant,
  removeAssistant,
  ...
  const reorderAssistants = useCallback(
    (fromSlotId: string, toSlotId: string) => {
      if (membershipMutationBusy || relayingSlotIds.size > 0 || fromSlotId === toSlotId) return;
    ...
    [membershipMutationBusy, relayingSlotIds]
  );
