...
use crate::types::{MailboxMessageType, TeammateRole, TeammateStatus, truncate_last_message};
...
impl TeammateManager {
    pub async fn set_status(&self, slot_id: &str, status: TeammateStatus) -> Result<(), TeamError> {
        let last_message = {
            ...
            matches!(status, TeammateStatus::Idle | TeammateStatus::Completed)
                .then(|| slot.agent.last_message.clone())
                .flatten()
        };
        self.events.broadcast_agent_status(slot_id, status, last_message);
        debug!(team_id = %self.team_id, slot_id, %status, "agent status changed");
        Ok(())
    }

    /// Records the agent's latest group-chat reply on the in-memory slot so
    /// subsequent settled-status broadcasts can carry it. Unknown slots are
    /// ignored.
    pub async fn set_last_message(&self, slot_id: &str, content: &str) {
        let mut slots = self.slots.lock().await;
        if let Some(slot) = slots.get_mut(slot_id) {
            slot.agent.last_message = Some(truncate_last_message(content));
            slot.agent.last_message_at = Some(now_ms());
        }
    }
