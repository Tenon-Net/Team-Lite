...
impl TeamAgentProvisioner {
    ...
    async fn create_team_conversation_for_agent(
        ...
        let (top_level_model, extra) = if agent_type == AgentType::Aionrs {
            ...
            let mut extra = extra;
            if !backend.eq_ignore_ascii_case("grok") || routed_to_claude {
    ...
    async fn resolve_claude_provider_for_model(&self, model: &str) -> Result<Option<Provider>, TeamError> {
        let providers = self.provider_repo.list().await?;
        Ok(select_claude_provider_for_model(providers, model))
    }
}

fn select_claude_provider_for_model(providers: Vec<Provider>, model: &str) -> Option<Provider> {
    let model = model.trim();
    let use_provider_default = model.is_empty() || matches!(model.to_ascii_lowercase().as_str(), "default" | "auto");
    let mut candidates: Vec<(Provider, bool, bool)> = providers
        .into_iter()
        ...
        .filter_map(|provider| {
            ...
            let model_enabled = provider
                .model_enabled
                .as_deref()
                .and_then(|raw| serde_json::from_str::<std::collections::HashMap<String, bool>>(raw).ok())
                .and_then(|enabled| enabled.get(model).copied())
                .unwrap_or(true);
            let exact_model =
                !use_provider_default && model_enabled && models.iter().any(|candidate| candidate == model);
            let protocols = provider
                .model_protocols
                .as_deref()
                .and_then(|raw| serde_json::from_str::<std::collections::HashMap<String, String>>(raw).ok())
                .unwrap_or_default();
            let exact_anthropic_protocol = exact_model
                && protocols
                    .get(model)
                    .is_some_and(|protocol| is_anthropic_protocol(protocol));
            let has_anthropic_protocol = protocols.values().any(|protocol| is_anthropic_protocol(protocol));
            let anthropic_platform = matches!(
                provider.platform.trim().to_ascii_lowercase().as_str(),
                "anthropic" | "claude"
            );
            let supports_default = use_provider_default && (anthropic_platform || has_anthropic_protocol);

            (exact_model || supports_default).then_some((
                provider,
                exact_anthropic_protocol || (use_provider_default && has_anthropic_protocol),
                anthropic_platform,
            ))
        })
        .collect();

    candidates.sort_by_key(|(_, anthropic_protocol, anthropic_platform)| (!*anthropic_protocol, !*anthropic_platform));
    candidates.into_iter().next().map(|(provider, _, _)| provider)
}
...
mod tests {
    ...
    fn provider_row(id: &str, platform: &str, models: &[&str]) -> Provider {
        Provider {
            id: id.into(),
    ...
    #[test]
    fn claude_team_provider_selection_prefers_anthropic_protocol_for_exact_model() {
        let generic = provider_row("generic", "openai-compatible", &["claude-sonnet-5"]);
        let mut cpa = provider_row("cpa", "new-api", &["claude-sonnet-5"]);
        cpa.model_protocols = Some(r#"{"claude-sonnet-5":"anthropic"}"#.into());

        let selected = select_claude_provider_for_model(vec![generic, cpa], "claude-sonnet-5").unwrap();

        assert_eq!(selected.id, "cpa");
    }

    #[test]
    fn claude_team_provider_selection_supports_cli_default_model() {
        let generic = provider_row("generic", "openai-compatible", &["gpt-5"]);
        let cpa = provider_row("cpa", "anthropic", &["claude-sonnet-5"]);

        let selected = select_claude_provider_for_model(vec![generic, cpa], "default").unwrap();

        assert_eq!(selected.id, "cpa");
    }

    #[test]
    fn claude_team_provider_selection_skips_disabled_exact_model() {
        let mut disabled = provider_row("disabled", "anthropic", &["claude-sonnet-5"]);
        disabled.model_enabled = Some(r#"{"claude-sonnet-5":false}"#.into());

        assert!(select_claude_provider_for_model(vec![disabled], "claude-sonnet-5").is_none());
    }
